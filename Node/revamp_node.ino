...
/* Node for soil water tension management system

     Purpose: Manage soil water tension by monitoring watermark 200ss sensors (& temperature) for temperature drift.
     Node sends data to a gateway for storage on sd card module / bluetooh to external PC. Node will initiate an irrigation
   ...
   who has had great influence on the present system.
*/
//-----Some future considerations------------------------------------------------------
/*
   1  connect individual temp sensors to groups of WM sensor or number 0-15 like WM sensors...
   2  transmission with gateway reliability has been spotty, tried increasing timeout to wait for an ACK 04/21/2022
   3  When sd card not present or fails, write to the flash chip?
   4  Adding on to the 4 channels of water thresholds... users should be able to set timing parameters for each irrigation group. Not necessary for the preliminary study taking place in Spring 2022
   5
   6  Parse menu option for radio settings returning registers (transmission strength etc.)
   7  The toggle radio and toggle troubleshooting menu options have been added but remain untested.
*/
//-----Libraries--------------------------------------------------
...
#include<DS3232RTC.h>             //Precision (generic) RTC library for DS3232 rtc module, requires v 1.3 
...
#define DS18B20_pin 1                     // Data line for Ds18b20, note all ds18b20 data lines are common and lead through a fixed 4.7K resistor
#define in1 12                            //relay 1 on D12
#define in2 13                            //relay 2 on D13
#define in3 14                            //relay 3 on D14
#define in4 3                             //relay 4 on D03
...
//mux select pins for s0,s1,s2,s3 are 18,19,20,21
...
boolean Real_loop = true;              //Run Real loop = true OR false = run demo loop for troubleshooting (does not go to sleep) -> once eeprom trouble shooting routine is tested and OK, delete this option.
...
uint32_t last_irr_time_unix_epoch_group4;    //4th irrigation group was added 03/18/2022.
...
String data = "";
String data2 = "";                                     //Careful with strings, they use dynamic memory allocation. Do I need to specify the length of this, the same as Len2?? -> will not compile
String header = "";
String WM_string = "";
...
struct eeprom_struct {
  ...
  char projectID[numChars] = {0};        //Project identifier 0-255 ####11/18/2021 changed to char array
  ...
  //int water_threshold;                  //for defining water threshold 0-255, integer type ->> Changing to 4 threshold groups... 03/18/2022 delete line once confirmed..
  int water_threshold_1;                  //for defining water threshold for each group, 0-255, integer type
  int water_threshold_2;                  //for defining water threshold for each group, 0-255, integer type
  int water_threshold_3;                  //for defining water threshold for each group, 0-255, integer type
  int water_threshold_4;                  //for defining water threshold for each group, 0-255, integer type
  ...
  int fixed_resistor_val;                // The value of the fixed resistor attached in series to the sensor and ground(voltage divider circuit)
  int num_WM;                            // Define the number of watermark sensors present

  int WM_group1[16] {0};                //Array to hold mux channel location of WM sensors to average, can be any number of locations, 4 potential groups for calc here
...
DeviceAddress DS18B20_Address;           //Array to hold sensor addresses ->> This may only be specifying one address for all the sensors, may be causing errors. ->> this is defined as uint8_t in the dallastemperature library
...
void setup() {
  ...
  delay(100);
  //sensors.begin();                   //trying here for ds18b20 problem...
  SPI.begin();
  delay(300);
  ...
  pinMode(in1, OUTPUT);              //declare relay pins as outputs, pins will need changed as project develops
  pinMode(in2, OUTPUT);
  pinMode(in3, OUTPUT);
  pinMode(in4, OUTPUT);

  digitalWrite(in1, LOW);           //New 4ch relay we want normally lOW and activated when HIGH. Bridge the COM & HIGH pins on the relay with provided jumper. Provide Vcc & GND from 5v wall plug. connect microcontroller pins to relay switches. Connect common ground between microcontroller and 5v wall plug.
  ...
  //Second alarm here for timesync from gateway.
  //-----
  ...
  if (eeprom_object.demo_mode == false) {       //Check for troubleshooting/demo mode in the eeprom settings
    ...
    if (RTC.alarm(ALARM_1)) {                   //When alarm 1 goes off...
      ...
      if (eeprom_object.toggle_radio) {         //If Radio transmission is ENABLED (true)..
        transmitData();                         //Radio transmission subroutine
      }
      delay(50);
    ...
    //---Demo loop---------------------------
    //                              //An alarm interrupt has triggered
    //get_nist_time();                //Once a day get time from NIST (To be incorportated in the gateway once cellular module is integrated)
    //time_sync();                    //Once a day, listen for the gateway to transmit it's time, sync to gateway time.
    readRTC();                      //Read the RTC
    delay(50);
    //time_sync();                  //Check for time sync from gateway
    Temp = getTemp();               //Read DS18B20 temperature sensors
    delay(50);
    read_watermark();               //Read the WaterMark Sensors
    delay(50);
    ...
    compile();                      //Format data from subroutines into one string for transmission
    delay(50);
    // saveData();                     //Write the data string to the sdcard ->> 2022_05_19 Perhaps we do not want to save the data on the troubleshooting loop.
    delay(50);
    if (eeprom_object.toggle_radio) {         //If Radio transmission is ENABLED (true)..
      transmitData();                         //Radio transmission subroutine
    }
...
void water_manager() {
  ...
  */
  ...
  if (eeprom_object.is_water_manager_on == true) {                                    //if water manager is active...
    ...
    /*
        WM_irrigation_prompt(1, WM_group1_mean, last_irr_time_unix_epoch_group1);   -->> Delete this group of lines once complete
      WM_irrigation_prompt(2, WM_group2_mean, last_irr_time_unix_epoch_group2);
      WM_irrigation_prompt(3, WM_group3_mean, last_irr_time_unix_epoch_group3);
      WM_irrigation_prompt(4, WM_group4_mean, last_irr_time_unix_epoch_group4);
    */
  } else {
...
void read_watermark() {
  // Real code with watermarks---------------------------------

  //Declare vars
  //const int mux_1_pin = A0;                 //connect to signal pin of MUX 1
  ...
  float SupplyV = 3.3;                        //Assuming a 5v output, replaced with an exact value later or if using a different logic board (3.3V etc)
  //String WM_string;                         //To store the data string
  ...
  if (eeprom_object.calibration_resistor_present == true) {   // Read the calibration resistor IF included, no polarity swap needed here
    for (int j = 0; j < num_Iter; j++) {            //the num_Iter controls the number of successive read loops that is averaged.
      ...
      delay(10);
      digitalWrite(WM_path2, LOW);                 //Set pin 11 as gnd
      digitalWrite(WM_path1, HIGH);                 //Set pin 7 as Vs
      delay(0.09);                           //wait 90 micro seconds and take sensor read
      ARead_path1 += analogRead(WM_analog_read_pin);            //read the analog pin the multiplexor is connected to
    ...
    Calib_Resistance = 1;             //if not actually calculated, 1
  ...
  for (int i = 0; i < eeprom_object.num_WM; i++) { //Assuming num_WM starts at mux channel 0 and increments, i.e. if num_WM = 4, i=0,1,2,3
    ...
    //Conversion of Resistance to centibars/kilopascals, 6 cases plus faults
...
float getTemp() {
  ...
  delay(10);
  // printROM();                                                  //void function that spits out addresses of connected sensors with their temperature.
  //This is currently not in order with what was saved to eeprom...

  //found code to store the addresses in eeprom. This seems by-far the best solution through a menu() window to identify sensors....
  ...
  //Also getting sensor addresses as an array is most logical, can vary the length of array by the number of working sensors...
  ...
  for (int i = 0; i < (eeprom_object.num_ds18b20); i++) {
    ...
    if (single_temp == -127.00 || single_temp == -98.00 || single_temp == 85.00) {                                               //Check for known fault codes
      ...
      // sum_temp += 0;
      //num_working_ds18b20 -= 1;
      //Serial.print(F("Getting and using temperature from RTC instead... RTC Temp: "));
...
//-----Transmit Data------------------------------------------------ This is based off RH reliable datagram
void transmitData() {
  ...
  if (manager.init()) {
    driver.setFrequency(915.0);                                             //Reset radio parameters each time a call to transmit is made
    delay(10);
    driver.setTxPower(20, false);
    delay(10);
    Serial.println(F("Radio initilization successful."));
    Serial.print(F("RH MAX MESSAGE LENGTH:  "));
  ...
  byte Len1;   //bytes can only go from 0-255
  byte Len2;

  delay(10);

  if (data.length() > (RH_RF95_MAX_MESSAGE_LEN - 1)) {        //If the data string requires a second packet. May consider revising to calculate the number of packets req.
    Len2 = (abs(data.length() - RH_RF95_MAX_MESSAGE_LEN) + 3); //add 1 for the null at end of Len1 & 1 for the null at end of Len2 & 1 for ??
    Len1 = RH_RF95_MAX_MESSAGE_LEN - 1;                       //Force max message length if data is over max message length
  } else {
    Len1 = data.length() + 1;                                 //If under max message length, just the length of data +1 for null
    Len2 = 0;
  }
  delay(10);
  ...
  // Serial.print(F("crazy Pack1:  "));
  // Serial.println(Pack1);
  // Serial.print(F("crazy Pack2:  "));
  // Serial.println(Pack2);

  delay(10);

  if (Len2 != 0) {
    Serial.println(F("Len2 is not 0."));
    data2 = data.substring(249, (data.length() + 1));         //This may or may not work properly and needs testing 2022-04-01
  } else {
    Serial.println(F("Len2 is 0."));
    data2 = "";
  }
  ...
  data2.toCharArray(Pack2, Len2); //// <-----------------------This is not giving an empty array..... This should copy the string data2 to the char array Pack2 of size Len2

  delay(10);

  Serial.print(F("Pack1:  "));
  Serial.println(Pack1);
  Serial.print(F("Pack2:  "));
  Serial.println(Pack2);
  ...
  Serial.print(F("Data: "));
  Serial.println((char*)Data); //This is a duplicate print of the data string...
  Serial.print(F("Data2:  "));
  ...
  Serial.print(F("Current gatewayID:  "));
  Serial.println(eeprom_object.gatewayID);
  Serial.print(F("Current nodeID: "));
  Serial.println(eeprom_object.nodeID);

  if (Len2 > 0) {
    Serial.println(F("Sending two packets..."));
    if (manager.sendtoWait(Data, Len1, eeprom_object.gatewayID)) {
      Serial.println(F("sendtoWait packet 1 OK."));
    } else {
      Serial.println(F("sendtoWait packet 1 Failed."));
    }
    //delay(3000);
    if (manager.sendtoWait(Data2, Len2, eeprom_object.gatewayID)) {
      Serial.println(F("sendtoWait packet 2 OK."));
    } else {
      Serial.println(F("sendtoWait packet 2 Failed."));
    }
  } else {
    Serial.println(F("Sending single packet..."));
    if (manager.sendtoWait(Data, Len1, eeprom_object.gatewayID)) {
      Serial.println(F("sendtoWait single packet OK."));
    } else {
      Serial.println(F("sendtoWait single packet Failed."));
    }
  }
  ...
  // Serial.print(F("Number of transmitted packets:  "));
  // Serial.println(manager.txGood());                                     //Returns # of packets successfully transmitted (though not necessarily received by the destination) type = uint16_t
}
...
void compile() {
  ...
  header += hrs;
  header += ':';
  if (mins < 10) header += "0";                                                                 // this is working
  header += mins;
  header += ':';
  if (secs < 10) header += "0";
  header += secs;
...
void Low_Power_Sleep() {
  ...
  analogComp_off();                         // turn off analog comparator (ArduinoSoilH2O)
  ADC_off();                                // turn off ADC (ArduinoSoilH2O)
  JTAG_off();                               // disable On-Chip Debug system (ArduinoSoilH2O)
  set_sleep_mode(SLEEP_MODE_PWR_DOWN);      //Power down completely on sleep
  sleep_enable();
  setRTCInterrupt();
  sleep_mode();                             //Puts MEGA to sleep (ArduinoSoilH2O)
...
//------------- Power Savers (Directly from ArduinoSoilH2O) ---------------------
...
void menu() {
  ...
  //Serial.print(F("appending filename with:  "));
  //for(int i=0; i<4; i++){
  //  Serial.print(a[i]);
  //}
  //Serial.println();
  ...
  Serial.println(F("   a  <--  Set Alarm (measurement) Interval")); //Set the alarm intervals for the RTC
  Serial.println(F("   g  <--  Toggle Radio Transmissions (ON/OFF)")); //Enable or disable LoRa Radio transmisison
  Serial.println(F("   h  <--  Toggle Troubleshooting mode")); //Enter Troubleshooting continuous loop
  Serial.println(F("   b  <--  Identify connected ds18b20 sensors"));// menu routine for iteratively connecting temperature sensors and saving them to eeprom
  Serial.println(F("   t  <--  Test measurements"));             // take test measurements from sensors -> not currently used
  Serial.println(F("   s  <--  Switch Water Manager"));          // Toggle/Switch Water Manager routine
  Serial.println(F("   w  <--  Define water threshold values and times"));  // Define threshold to trigger an event
  Serial.println(F("   n  <--  Specify number of WM sensors used & Means grouping")); //Set number of WM sensors installed & specify groups based on channel position of multiplexor? Could also write funciton to subtract 1 from numbers specified so that actual terminal numbers can be used.
  Serial.println(F("   o  <--  Specify Resistors presence and values for WM multiplexor")); //Define resistor information
  Serial.println(F("   p  <--  Close Relay to Prime Pumps"));    // For priming the peristaltic pump lines
  //  Serial.println(F("   r  <--  Download range of data"));    // set beginning date to download, not functional
  Serial.println(F("   f  <--  Display sdcard information"));    // Get sdcard information
  Serial.println(F("   d  <--  Download all data"));             // get all data to serial port
  Serial.println(F("   e  <--  Erase all data"));                // erase all data
  //  Serial.println(F("   m  <--  Repeat menu"));               // repeat menu, not functional
  ...
  switch (menuinput) {                                          //switch cases for menu input, Note the case# corresponds to input in ASCII format
    case 98:                                                    // "b" for iterating connection of ds18b20 temperature sensors
      ...
      get_integer_input();
      // do not specify int user_resp = indata; // for some reason it does not allow menu access??
      int user_resp;
      delay(100);
      user_resp = indata;
      delay(100);
    ...
    case 112:                                                             // "p" for prime pumps--------------------------------
      ...
      if (charInput[0]) {
        ...
        if (strcmp(answer, "YES") == 0) {
          ...
          digitalWrite(in1, HIGH);                                  //provide power to pump on relay on respective pin, new relays are HIGH activated. pumps should be normally open & close when powered
          delay(10000);
          digitalWrite(in1, LOW);                                   //turn off pumps
          delay(2000);
          ...
          // unsigned long timeout = 10000;                             //defined 10 seconds for priming routine, 3 sec for now
    ...
    case 105:                                                    // "i" for set ID numbers-----------------------------------------
      ...
      //attempt incorporation of charinput funciton
      charinput();
      if (charInput[0]) {
        byte i = 0;
      ...
      manager.setThisAddress(eeprom_object.nodeID);                         // ArduinoSoilH2O radio library method
    ...
    case 119:                                                     // "w" for setting water threshold levels and min time between irrigation events and toggleing reporting of resistance values
      ...
      get_integer_input(); //otherwise the first input is always 0?
      Serial.println();
      Serial.print(F("Current Value Group 1: "));                 // updated 03/18/2022 to include 4 groups of threshold levels
      Serial.println(eeprom_object.water_threshold_1);
      get_integer_input();
      ...
      Serial.println(F("Duration of irrigation event: "));        //define duration of irrigation event (units in millis i.e. 10 sec is 10,000millis)
      Serial.println();
      Serial.print(F("Current Value:  "));
      Serial.println(eeprom_object.irr_period);
      get_integer_input();
    ...
    case 100:                                                     // "d" for Download all data in sd card----------------------------
      ...
      if (myfile) {
        ...
        while (myfile.available()) {                              // read file and print to screen (COMPORT), Note this will be slow with alot of data due to chip limitations. A desktop with a chip reader is nearly instantaneous.
    ...
    case 110:                                                   //"n" for sepecifying number of WM sensors & grouping | 11/18/2021 untested, not sure if will work
      ...
      /*
          //check for positions specified by user....
               for (int i = 0; i < num_sensors; i++) {
        Serial.print(F("eeprom_object.WM_group1 position  "));
        Serial.print(i);
        Serial.print(F(" = channel "));
        Serial.println(eeprom_object.WM_group1[i]);
        }
      */
...
//-----Get User input as integer----------------------------------------------- 11/18/2021 currently does not accept groups for means handling i.e. 1,2,3 or 4,5,6 etc. indata is an integer and will only return 0-255
...
//-----WM Group means handling------------------                                 //This portion could probably be substantially optimized by a programmer
...
int WM_group_means(int WM_group_num[], int datasize, int water_threshold_group) {
  ...
  while (pointer != NULL) {
    ...
    index++;
    // Serial.print(F("pointer[index]:  "));
    // Serial.println(pointer[index]);
    // Serial.print(F("pointer_array[index]: "));
    // Serial.println(pointer_array[index]);
  ...
  /*
       Serial.print(F("WM_string:  "));
    Serial.println(WM_string);

    for (int g; g < sizeof(WM_array); g++) {
      Serial.print(WM_array[g]);
    }
    Serial.println();
  ...
  */
  for (int i = 0; i < datasize; i++) {                                           //for the declared channel group,
    ...
    if (q != ',' && q != ' ' && q != '-' && q != "-1" && q <= 16) {                  //if q is not a comma or a space, -1, or some random number above 16...
      ...
      // Serial.print(F("Pointer array[q]: "));
      // Serial.println(atoi(pointer_array[q]));
      int wm_val;
      ...
      if (WM_group_num[i] != -1 && WM_group_num <= 16 && wm_val != 255 && wm_val != 240 && wm_val != 0) { //-1 must be specified as "-1"? maybe not?
  ...
  for (int i = 0; i < datasize; i++) {                                           //for the declared channel group,
    ...
    if (q != ',' && q != ' ' && q != '-' && q != "-1" && q <= 16) {                  //if q is not a comma or a space, -1, or some random number above 16...
      ...
      // Serial.print(F("Pointer array[q]: "));                                        //the wm_val for the indicated sensor
      // Serial.println(atoi(pointer_array[q]));
      int wm_val;
      wm_val = atoi(pointer_array[q]);
      ...
      if (wm_val != 255 && wm_val != 256 && wm_val != 240 && wm_val != 0 && WM_group_num[i] != -1) {
        if (abs((abs(raw_group_mean - wm_val) / ((raw_group_mean + wm_val) / 2.0)) * 100) < 20.0) { //if the sensor reading is less than 20% different from the raw_group_mean, include in calc of buffered_group_mean **********04/25/22
          Serial.print(F("Channel: "));
          Serial.print(WM_group_num[i]);
          ...
          /// Resume here after 04/25/22, -31 was - 10 diff from raw mean but should have fallen within +-10 of the
...
//-----Water manager irrigation prompt-------------------------------------------------------- ->>> 03/18/2022 this needs updated to work with the 4 groups of thresholds now... Perhaps can write it as a loop to run through the 4 groups.
...
uint32_t WM_irrigation_prompt(int WM_group_num, int WM_group_mean, int WM_group_water_threshold, uint32_t last_irr_time_for_group) { // The way this is setup as a function, you just run it for each group, no loop req.
  ...
  if (WM_group_num == 1) {
    if (WM_group_mean < eeprom_object.water_threshold_1) {                          //if sensors indicate need for watering event...
      ...
      DateTime now = rtc.now();                                                   //needed to get unix time in next line
      uint32_t current_unix_epoch_time = now.unixtime();                          //get current unix epoch time
      //Serial.print(F("Current unix epoch time is: "));
      ...
      if (current_unix_epoch_time - last_irr_time_for_group >= (eeprom_object.min_time_btwn_irr * 60)) {  //IF the time since last irrigation event is greater than or equal to the minnimum time between irrigations (seconds*60 = min)...
        Serial.println(F("The minimum time since last irrigation event has been exceeded. Proceed with irrigation")); // 2022/03/22 Note that this will need changed in future if separate timing differences are specified for each group...
        ...
        irrigation_prompt_string += ',';
        // irrigation_prompt_string += update_last_irr_time;
        ...
        //irr_count ++;                                     //like incrementing irr_count for throwing flag if X events take place in Y time?
...
//
/*
    //OLD code for a single water_threshold defined...
     // note millis is not incremented during sleep, so last irr time will need kept track of using the RTC, probably using seconds since Unix epoch

   uint32_t WM_irrigation_prompt(int WM_group_num, int WM_group_mean, uint32_t last_irr_time_for_group) {    //Need to specify which group and the last irrigation time for that group

     uint32_t update_last_irr_time;                                                //an updated last irr time is needed.
     if (WM_group_mean < eeprom_object.water_threshold) {                          //if sensors indicate need for watering event...
       Serial.println(F("Need for watering event indicated."));
       DateTime now = rtc.now();                                                   //needed to get unix time in next line
       uint32_t current_unix_epoch_time = now.unixtime();                          //get current unix epoch time
       //Serial.print(F("Current unix epoch time is: "));
       //Serial.println(current_unix_epoch_time);

       // If watering event is needed, check how long it has been since the last irrigation event...
       if (current_unix_epoch_time - last_irr_time_for_group >= (eeprom_object.min_time_btwn_irr)) {    //IF the time since last irrigation event is greater than or equal to the minnimum time between irrigations (seconds)...
         Serial.println(F("The minimum time since last irrigation event has been exceeded. Proceed with irrigation"));
         //use case statement to toggle the relays, each group mean 1-3 will be connected to corresponding relay in1,in2,in3
         int relay_case = WM_group_num;
         switch (relay_case) {
           case 1:
             digitalWrite(in1, HIGH);                                   //provide power to pump on relay on respective pin
             delay(eeprom_object.irr_period * 1000);                   //defined length/duration of irrigation event (seconds so * 1000)
         ...
         irrigation_prompt_string += ',';
         //different here than in case above
         //irrigation_prompt_string += last_irr_time_for_group;            //if minimum time between irrigations has not been exceeded, return the time of last irrigation event for the group
         //local_time(last_irr_time_for_group);
       ...
       Serial.print(WM_group_mean);
       Serial.print(F(", Threshold water content of "));
       Serial.print(eeprom_object.water_threshold);
       Serial.println(F("  has not been exceeded."));
     ...
     /*
       ...
       Serial.println(F("END Water Manager Loop."));
       Serial.println();
       /  // insert * / here to use this old code

   }
*/

//----testing print ROM of ds18b20----------------------------------------------- 04/05/2022
void printROM() {
  for (int i = 0; i <= 15; i++) {
    Serial.print(F("Sensor  "));
    Serial.print(i);
    ...
    //  if ((sensors.getAddress(DS18B20_Address, (i))) == (sensors.getAddress(DS18B20_Address, (i+1)))) { //for some unknown reason this does not work... so the last sensor address will be repeated.
    //    Serial.println(F("No additional unique sensor addresses found."));
    //   break;
    //  }
  }
}
...
//------Calc battery voltage (0- ~ 10V) with voltage divider circuit------------- 11/24/2021 This works, closed/opened by mosfet with digital input to prevent battery drain. Not certain on application
...
void local_time(uint32_t unix_time) {
  ...
  */
  ...
  temp.toCharArray(local_time_irr_update, numChars); // works.

  //Serial.print(F("local_time_irr_update:  "));
...
//Import measurement interval from connected gateway
...
void time_sync() {
  ...
  if (manager.recvfromAck(buf, &len, &from_id)) {
    ...
    //What to do about getting from unix to std format?
    ...
    while (oneWire.search(addr)) {
      ...
      switch (intSensorsPassed) {
        ...
        case 1:
          ...
          //Serial.println("Incoming address = "); printAddress(addr);
...
//-----Getting temperature sensor data into the data string-----------------------------------
void connected_temp_to_string() {}
